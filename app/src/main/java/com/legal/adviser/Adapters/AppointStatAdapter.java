package com.legal.adviser.Adapters;
...
import com.legal.adviser.ApoointmentComplete;
import com.legal.adviser.ClientSide.AppointmentStat;
import com.legal.adviser.ClientSide.ClientDashboard;
import com.legal.adviser.Models.AppointmentModel;
import com.legal.adviser.R;
import com.legal.adviser.SendNotificationPack.APIService;
import com.legal.adviser.SendNotificationPack.Client;
import com.legal.adviser.SendNotificationPack.Data;
import com.legal.adviser.SendNotificationPack.MyResponse;
import com.legal.adviser.SendNotificationPack.NotificationSender;
...
//                                    UpdateAppointStatus("Finish",model.getAppintKey(),model.getLawyerID(),model.getClientID(),model.getAppintKey());
                                    ...
                                    DatabaseReference ref1= FirebaseDatabase.getInstance().getReference().child("CaseRequests").child(model.getAppintKey());
                                    DatabaseReference ref2= FirebaseDatabase.getInstance().getReference().child("Appointments").child(model.getAppintKey());
                                    ref1.removeValue();
                                    ref2.removeValue();

                                    Toast.makeText(ctx, "Request Finish", Toast.LENGTH_SHORT).show();
                                    Intent i = new Intent(ctx, ApoointmentComplete.class);
                                    i.putExtra("LKEY",model.getLawyerID());
                                    i.putExtra("clientID",model.getClientID());
                                    i.putExtra("appintKey",model.getAppintKey());
                                    ctx.startActivity(i);
...
//                                    UpdateAppointStatus("Canceled",model.getAppintKey(),model.getLawyerID(),model.getClientID(),model.getAppintKey());
                                    ...
                                    dialog.cancel();
                                    ctx.startActivity(new Intent(ctx, AppointmentStat.class));
                                    ((Activity)ctx).finish();
        ...
        ref.updateChildren(updates).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if(task.isSuccessful()){
                    ...
                    Toast.makeText(ctx, "Request "+sts, Toast.LENGTH_SHORT).show();
                    Intent i = new Intent(ctx, ApoointmentComplete.class);
