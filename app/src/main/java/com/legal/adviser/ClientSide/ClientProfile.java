                    ...
                    public void onComplete(@NonNull Task<Void> task) {
                        if (task.isSuccessful()) {
                            auth.getCurrentUser().updatePassword(password).addOnCompleteListener(new OnCompleteListener<Void>() {
                                ...
                                public void onComplete(@NonNull Task<Void> task) {
                                    if (task.isSuccessful()) {
                                        Toast.makeText(ClientProfile.this, "Password updated", Toast.LENGTH_SHORT).show();
                                        UpdateData(name,phone,address,password);
                                    } else {
                                        progressDialog.dismiss();
                                        Toast.makeText(ClientProfile.this, "Error password not updated", Toast.LENGTH_SHORT).show();
                                    }
                                }
                            });
                        } else {
                            progressDialog.dismiss();
                            Toast.makeText(ClientProfile.this, "Error auth failed", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
    }
        ...
        ref.updateChildren(updates).addOnCompleteListener(new OnCompleteListener<Void>() {
            ...
            public void onComplete(@NonNull Task<Void> task) {
                if(task.isSuccessful()) {
                    ...
                    progressDialog.dismiss();
                    Toast.makeText(ClientProfile.this, "Failed", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
