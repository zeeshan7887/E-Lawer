...
import com.legal.adviser.Adapters.UserAdapter;
import com.legal.adviser.Models.LawyerModel;
                            ...
                            if(uList.size() != 0){
                                for(LawyerModel model : uList){
                                    if(!snapshot1.getKey().equals(model.getKey())){
                                        uList.add(new LawyerModel(snapshot1.getKey(),"fpv3_8mUq90:APA91bFlDtwohARNfcZZcGrEQbY_LTNmKqyWASrJk_2Ws19KD75b40KBIpz-z9qtLIkmR4hEukVtY8-fnwk1KHGCHhpBksNe_pBGs-gpvJ0_VgT7qsjR-3Edhs4wN7IeCNYcSBrYG21o",name,email,password,phone,address,status,imgURL,dateTime));
                                    }
                                }
                            }
                            else{
